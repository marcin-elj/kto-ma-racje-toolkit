...
description: Use when searching a codebase to pick the right tool for the job — rg for exact text, semantic search for concepts, symbol-aware tools for refactoring, AST tools for structural patterns. Includes decision tree, combined workflows, and pitfalls (e.g. don't use semantic search for known function names).
---
