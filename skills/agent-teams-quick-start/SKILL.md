...
description: Use when planning multi-perspective audits, security PR reviews, pre-release checklists, multi-file doc updates, or architecture reviews that benefit from running 2-4 specialized agents in parallel. Provides decision matrix for when parallel agents are worth the 3x token cost vs sequential execution.
---
